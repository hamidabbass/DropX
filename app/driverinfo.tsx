...
export default function DriverInfoScreen() {
  ...
  React.useEffect(() => {
    const hydrate = async () => {
      try {
        ...
        const access =
          loginData?.access || (await AsyncStorage.getItem("access"));
        if (!access) return;
        ...
        if (!driver?.cnic || !driver?.name) {
          try {
            const verRes = await fetch(
              `${API_BASE_URL}/driver-verification/verifications/`,
              {
                headers: { ...authHeader },
              }
            );
            if (verRes.ok) {
              ...
              const record = Array.isArray(data)
                ...
                : data; // last (latest) fallback first
              if (record) {
                const full_name =
                  record.full_name || record.name || driverRegister?.full_name;
                const cnic_number = record.cnic_number || record.cnic;
                const driverPhoto =
                  record.photo ||
                  record.cnic_photo ||
                  record.license_photo ||
                  null;
  ...
  return (
    ...
    >
      ...
      >
        <Text style={tw`text-2xl font-bold text-black mt-6 mb-4`}>
          Driver Info
        </Text>
        {/* Driver Photo */}
        <View style={tw`items-center mb-6 mt-2`}>
          {driver?.photo ? (
            <Image
              ...
              style={tw`size-44 rounded-full`}
            />
          ) : (
            <View
              style={tw`size-44 bg-gray-200 rounded-full items-center justify-center`}
            >
